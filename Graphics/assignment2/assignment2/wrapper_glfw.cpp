...
wrapper_glfw::wrapper_glfw(int width, int height, char *title)
{
	this->width = width;
	this->height = height;
	this->title = title;
...
GLFWwindow* wrapper_glfw::getWindow()
{
	return window;
}
...
void wrapper_glfw::setErrorCallback(void(*func)(int error, const char* description))
{
...
void wrapper_glfw::setRenderer(void(*func)()) {
	this->renderer = func;
}

/* Register a callback that runs after the window gets resized */
void wrapper_glfw::setReshapeCallback(void(*func)(GLFWwindow* window, int w, int h)) {
	reshape = func;
...
void wrapper_glfw::setKeyCallback(void(*func)(GLFWwindow* window, int key, int scancode, int action, int mods))
{
	keyCallBack = func;
...
GLuint wrapper_glfw::BuildShader(GLenum eShaderType, const std::string &shaderText)
{
...
std::string wrapper_glfw::readFile(const char *filePath)
{
	std::string content;
...
GLuint wrapper_glfw::LoadShader(const char *vertex_path, const char *fragment_path)
{
...
/* Load vertex and fragment shader and return the compiled program */
GLuint wrapper_glfw::BuildShaderProgram(std::string vertShaderStr, std::string fragShaderStr)
{
	GLuint vertShader, fragShader;
	GLint result = GL_FALSE;
