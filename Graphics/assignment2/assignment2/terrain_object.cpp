...
void terrain_object::createObject()
{
	...
	glBufferData(GL_ARRAY_BUFFER, xsize * zsize  * sizeof(glm::vec4), &(vertices[0]), GL_STATIC_DRAW);
	...
	glBufferData(GL_ARRAY_BUFFER, xsize * zsize  * sizeof(glm::vec3), &(textures[0]), GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
...
void terrain_object::calculateNoise()
{
	...
	GLint size = xsize * zsize * perlin_octaves;
	noise = new GLfloat[size];
	...
	for (int row = 0; row < zsize; row++) //each row
	{
		for (int col = 0; col < xsize; col++) //each column
		{
			...
			for (int oct = 0; oct < perlin_octaves; oct++)//each octave
			{
...
/* Define the vertex array that specifies the terrain
   (xp, zp) specifies the pixel dimensions of the heightfield (x * z) vertices
   (xs, zs) specifies the size of the heightfield region in world coords
   */
void terrain_object::createTerrain(GLuint xp, GLuint zp, GLfloat xs, GLfloat zs)
{
	...
	for (GLuint x = 0; x < xsize; x++)
	{
		...
		for (GLuint z = 0; z < zsize; z++)
		{
			...
			vertices[x*xsize + z] = glm::vec4(xpos, (height-0.5f)*height_scale, zpos, 1.0);
...
void terrain_object::calculateNormals()
{
	...
	for (GLuint x = 0; x < xsize - 1; x++)
	{
		...
		for (GLuint tri = 0; tri < zsize * 2 - 2; tri++)
		{
			...
			glm::vec3 vert1 = glm::vec3(vertices[v1].x, vertices[v1].y, vertices[v1].z);
...
void terrain_object::stretchToRange(GLfloat min, GLfloat max)
{
	...
	for (int v = 1; v < xsize*zsize; v++) //go through each vertice in the array and find the max and min values. 
	{
