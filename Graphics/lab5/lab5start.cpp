...
void init(GLWrapper *glw)
{
	...
	freq = 1.0f;
	scale = 2.0f;
	angle_inc_y = angle_inc_z = 0;
	aspect_ratio = 1.3333f;
	colourmode = 0;
	numlats = 20;		// Number of latitudes in our sphere
	numlongs = 20;		// Number of longitudes in our sphere

	//cone.load_obj("monkey.obj");
	//cone.load_obj("cone.obj");
	//cone.smoothNormals();
	//cone.createObject();

	terrain.createTerrain(100, 100, 20.0f, 20.0f, 1.0f, 2.0f);
...
void display()
{
	...
	glm::mat4 View = glm::lookAt(
		glm::vec3(0, 0, 40), // Camera is at (0,0,4), in World Space
...
static void keyCallback(GLFWwindow* window, int key, int s, int action, int mods)
{
	...
	if (key == 'A') scale -= 0.05f;
	if (key == 'S') scale += 0.05f;
	if (key == 'Z') x -= 0.05f;
	if (key == 'X') x += 0.05f;
	if (key == 'C') y -= 0.05f;
	if (key == 'V') y += 0.05f;
	...
	{
		terrain.createTerrain(100, 100, 20.0f, 20.0f, freq, scale);
		terrain.createObject();
	}
