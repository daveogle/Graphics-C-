/*
 Texture_multi
 Basic example to show multi-textures
 Combines a checkerboard texture pattern and an image loaded using SOIL.
 Contains diffuse lighting and scale, translation and rotation keyboard controls.
...
/* Uniforms*/
GLuint modelID, viewID, projectionID, tex_matrixID;
GLuint colourmodeID;
...
void init(GLWrapper *glw)
{
	...
	try
	{
		program = glw->LoadShader("texture_multi.vert", "texture_multi.frag");
	}
	...
	tex_matrixID = glGetUniformLocation(program, "tex_matrix");
	...
	GL_RED is used to specify one colour component, we use this to make white using the swizzle below */
	...
	try
	{
		...
		texID2 = SOIL_load_OGL_texture("img.png", SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID,
...
void display()
{
	...
	// Define a texture transform matrix
...
static void keyCallback(GLFWwindow* window, int key, int s, int action, int mods)
{
	...
	if (key == '1') tangle_x -= 0.5f;
	if (key == '2') tangle_x += 0.5f;
	if (key == '3') tangle_y -= 0.5f;
	if (key == '4') tangle_y += 0.5f;
	if (key == '5') tangle_z -= 0.5f;
	if (key == '6') tangle_z += 0.5f;
